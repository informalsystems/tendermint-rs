...
use crate::tester::TestResult::{Success, Failure, ParseError, ReadError};
...
impl TestResult {
    pub fn is_success(&self) -> bool {
        match self { TestResult::Success(_) => true, _ => false }
    }
    pub fn is_failure(&self) -> bool {
        match self { TestResult::Failure{ message: _, location: _ } => true, _ => false }
    }
    pub fn is_readerror(&self) -> bool {
        match self { TestResult::ReadError => true, _ => false }
    }
    pub fn is_parseerror(&self) -> bool {
        match self { TestResult::ParseError => true, _ => false }
    }
}
...
impl Tester {
    ...
    where F: FnOnce(TestEnv) -> TestEnv + UnwindSafe
    {
            ...
            T: 'static + DeserializeOwned + UnwindSafe,
    {
        ...
        let test_fn = move |input: &str| {
            match parse_as::<T>(&input) {
    ...
    fn add_result(&mut self, name: &str, path: &str, result: TestResult) {
        self.results.entry(name.to_string()).or_insert(Vec::new()).
            push((path.to_string(), result))
    }

    fn read_error(&mut self, path: &str) {
        self.results.entry("".to_string()).or_insert(Vec::new()).
