...
- `Verify` implements the checks 
  ...
  the assumption [TMBC-FM-2THIRDS]
...
- If *primary* is correct, 
    ...
    - `Verify` either verify the header or fall back to sequential
      verification
    - Eventually every header will be verified
    - **TODO:** this must happen before the *trustingPeriod* expires
- If *primary* is faulty,
    - it either provides headers that pass all the tests, and we
      return with the header
	- it provides one header that fails a test
	- **TODO:** also, be more precise here!
