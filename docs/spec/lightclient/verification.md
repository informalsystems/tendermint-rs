...
height *targetHeight*. Each instance must eventually terminate. 
  - If
     ...
     - *trustedStore* always contains a header whose age is less than the
        trusting period,  
...
He we provide a specification for Light Client Verification. The local
...
We highlight that if a different concurrency model is considered for
...
- `Pivot` decides which height to try to verify next. We keep this
  underspecified as different implementation (currently in Goland and
