---------------------- MODULE Tendermint4n5f1T3 -----------------------------
(*
 A TLA+ specification by Igor Konnov
 of Tendermint consensus by Ethan Buchman, Jae Kwon, and Zarko Milosevic.
 
 For the moment, we assume the following:
 
   1. Every process has the voting power of 1 (this seems to be complete).
   2. Timeouts are non-deterministic (works for safety).
   3. The proposer function is completely non-deterministic (works for safety).
 
 It took me 4 hours to translate the pseudo-code to TLA+.
 Then it took me several days to make it more efficient for the model checker.

 Version 3: assuming that f = 1,
    using AtLeastT1(S) and AtLeastT2(s) instead of
    Cardinality(S) >= f + 1 and Cardinality(S) >= 2f + 1

 Version 4: simplified record types, as we are using separate variables
    for different message types and different indices for heights and rounds.
    Injecting all Byzantine messages in the initial states.
 *)
...
\* If T is different from 1, use the general operators:
\*AtLeastT1(S) == Cardinality(S) >= THRESHOLD1
\*AtLeastT2(S) == Cardinality(S) >= THRESHOLD2
AtLeastT1(S) == \E x, y \in S: x /= y
AtLeastT2(S) == \E w, x, y, z \in S:
    x /= y /\ x /= z /\ y /= z /\ w /= x /\ w /= y /\ w /= z
