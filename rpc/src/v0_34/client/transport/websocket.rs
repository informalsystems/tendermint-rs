...
use crate::client::transport::router::{PublishResult, SubscriptionId, SubscriptionIdRef};
use crate::client::transport::websocket::{WebSocketClientUrl, WebSocketConfig};
use crate::v0_34::client::subscription::{Subscription, SubscriptionClient, SubscriptionTx};
use crate::v0_34::client::transport::router::SubscriptionRouter;
...
use crate::{
    ...
    query::Query,
    request::Wrapper,
    response, Client, Id, Request, Response, SimpleRequest, Url,
};
...
impl WebSocketClient {
        ...
        U: TryInto<WebSocketClientUrl, Error = Error>,
    {
        ...
        let (inner, driver) = if url.is_secure() {
            sealed::WebSocketClient::new_secure(url, config).await?
        } else {
            sealed::WebSocketClient::new_unsecure(url, config).await?
        };
...
mod sealed {
    ...
    use crate::v0_34::Subscription;
    use crate::{
        client::sync::{unbounded, ChannelTx},
        prelude::*,
        query::Query,
        request::Wrapper,
        utils::uuid_str,
        Error, Response, SimpleRequest, Url,
    };
