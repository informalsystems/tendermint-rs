...
fn outgoing_fixtures() {
    for json_file in find_fixtures("v0_38", "outgoing") {
...
fn incoming_fixtures() {
    use tendermint_rpc::event::{latest::DialectEvent, EventData};
    ...
    for json_file in find_fixtures("v0_38", "incoming") {
        let file_name = json_file
            .file_name()
            .unwrap()
            .to_str()
        ...
        match file_name {
            "abci_info" => {
                ...
                assert_eq!(result.response.version, "2.0.0");
            },
            "abci_query_with_existing_key" => {
            ...
            "blockchain_from_1_to_10" => {
                ...
                for block_meta in result.block_metas {
                    ...
                    if block_meta.header.height.value() == 1 {
                        assert_eq!(block_meta.header.app_hash.as_bytes(), &[0u8; 8]);
            ...
            "subscribe_txs_0" => {
                let result = DialectEvent::from_string(content).unwrap();
                let height;
                if let EventData::Tx { tx_result } = result.data.into() {
