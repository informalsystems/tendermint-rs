...
impl Set {
    ...
    pub fn new(mut vals: Vec<Info>) -> Self {
        vals.sort_by(|v1, v2| {
            v1.address
                .partial_cmp(&v2.address)
                .expect("validator address comparison failed")
        });
...
mod tests {
    ...
    fn make_validator(pk_string: &str, vp: u64) -> Info {
        let bytes = &hex::decode_upper(pk_string).expect("decoding key string failed");
        let pk = PublicKey::from_raw_ed25519(bytes).expect("decoding public key failed");
