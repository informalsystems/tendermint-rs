...
pub enum ErrorKind {
    ...
    #[error("header hash does not match the hash in the commit ({:?}!={:?})", .header_hash, .commit_hash)]
    InvalidCommitValue {
        header_hash: Hash,
        commit_hash: Hash,
    ...
    #[error("signed voting power ({}) do not account for +2/3 of the total voting power: ({})",
        .signed, .total
    )]
