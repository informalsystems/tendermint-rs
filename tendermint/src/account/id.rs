//! Tendermint account IDs: raw key fingerprints
...
/// Account IDs: raw account key fingerprints.
///
/// The [`Address`][`super::Address`] type provides a Bech32-encoded serialization.
#[derive(Copy, Clone, Eq, Hash, PartialEq, PartialOrd, Ord)]
pub struct Id(pub [u8; Self::LENGTH]); // JSON custom serialization for priv_validator_key.json
...
impl Id {
    ...
    /// Create a new account ID from the provided byte slice.
    ///
    /// Returns an error if the slice's length is not equal to [`Self::LENGTH`].
    pub fn new(bytes: &[u8]) -> Result<Self, Error> {
        bytes
            .try_into()
            .map(Self)
            .map_err(|_| Kind::InvalidAccountIdLength.into())
    }
...
impl From<&Secp256k1> for Id {
    fn from(pk: &Secp256k1) -> Id {
        ...
        bytes.copy_from_slice(&ripemd_digest[..Self::LENGTH]);
        Id(bytes)
    }
}
...
impl From<&Ed25519> for Id {
    fn from(pk: &Ed25519) -> Id {
        ...
        Id(digest[..Self::LENGTH].try_into().unwrap())
    }
}
...
// TODO: remove custom serialization?
